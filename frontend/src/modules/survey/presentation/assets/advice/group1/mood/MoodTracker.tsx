    ...
    <div className="space-y-4 rounded-xl border-2 border-pink-200 bg-pink-50/30 p-4">
      <div>
        <ul className="space-y-1 text-sm text-gray-700">
          <li className="flex items-center gap-2">
            ...
            <span className="text-day-red text-lg font-bold">ردیابی خلق‌ و خو</span>
          </li>
      ...
      <div className="flex gap-8">
        <div className="flex items-center">
          <span className="text-base font-semibold text-gray-400 [writing-mode:vertical-rl] rotate-360">
            ردیابی خلق ‌و خو
          </span>
        </div>

        <div className="flex-1 space-y-3">
          <div className="flex justify-between border-b pb-2 text-sm font-semibold text-day-primary">
            {WEEK_DAYS.map((day, i) => (
          ...
          {MOOD_ICONS.map((mood, moodIndex) => (
            <div key={moodIndex} className="flex items-center justify-between">
              {WEEK_DAYS.map((_, dayIndex) => (
                ...
                >
                  <img src={mood.image} alt={mood.label} className="h-12 w-12 object-contain" />
